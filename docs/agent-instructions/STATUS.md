...
- 🚨 **Найдено по ходу (21.09, вечер) — требует решения владельца:**
  ...
  - **Ложная тревога, которую стоит запомнить:** после моих прогонов число файлов в
    ...
    = 5 с 19.09 16:45 без провалов, в продовом Postgres те же 5 строк, а SQLite-зеркало
    совпадает с копии 16:01 до строки. Тестовый набор безопасен по умолчанию: `tests/conftest.py`
