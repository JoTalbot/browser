...
## 🟢 Последняя запись (2026-09-21, шаг 108 — ссылка «на запись» и порядок на ноде)
...
- 🚨 **Найдено по ходу (21.09, вечер) — требует решения владельца:**
  - **Хранилище хронически недореплицировано:** `immortal_lost_blocks = 3`,
    `immortal_degraded_blocks = 3`, у `uguu` здоровье 0.05 сутки. Причина не в железе, а в
    устройстве починки: `Store.repair` возвращает недостающему фрагменту **ту же** площадку,
    где он и пропал, а `uguu.space` не принимает объекты больше ~1 МБ. Отсюда вечный цикл
    «блок деградировал → починка не может записать → блок деградирован». Починка обязана
    пересаживать фрагмент на здоровую площадку, меняя `provider`, но **не меняя `index`**
    (криптография привязана к индексу, не к площадке). Это и есть шаг 109.
  - **Два демо-файла восстановить нельзя:** `Демо/заметка.txt` (62 Б) и `Демо/картинка.png`
    (5,4 КБ) плюс их `_manifests.jsonl` — у каждого блока выжила 1 копия из 5 при k=3,
    это меньше порога Reed–Solomon, math не даёт собрать байты. `live.bin` и `cas-object.bin`
    читаются и переживут починку. Решение за владельцем: перезалить демо-файлы с исходника
  ...
  - **Ложная тревога, которую стоит запомнить:** после прогонов тестов число файлов в
    `GET /v1/stats` казалось уменьшившимся (13 → 5). Проверка по `immortal_files_total`
    за 4 дня (значение 5 с 19.09 16:45) и побайтовое сравнение SQLite-зеркала с копиями
    12:00 и 16:01 показали: ничего не терялось. Но сам риск реален: `db.dsn_from_env()`
    при пустом окружении **сам читает `/etc/octopus/immortal-pg.env`**, то есть `pytest`
    без `IMM_DB` и без DSN пойдёт не в временную базу, а в прод. Правильно так:
    `env IMM_DB=$(mktemp -d)/l.db -u IMMORTAL_PG_DSN -u IMM_PG_DSN python -m pytest -q tests`.
